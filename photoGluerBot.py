from src.photoGluer import PhotoGluer
...
class TelegramBot:
    def __init__(self, token, temp_dir):
        ...
        self._dp.register_message_handler(
            self.handle_glue, commands=['glue'])
        self._dp.register_message_handler(
    ...
    async def handle_glue(self, message):
        await message.answer("Склеивание фото может занять некоторое время. Ожидайте.")

        res = self.glue(message.from_user.id)
        if not res:
    ...
    def glue(self, user_id):
        path = os.path.join(self._temp_dir, str(user_id))
        res = self._gluer.glue_vertical(
            path,
            path,
